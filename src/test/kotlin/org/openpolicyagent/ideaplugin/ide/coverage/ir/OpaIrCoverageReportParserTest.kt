...
class OpaIrCoverageReportParserTest {
    ...
    @Test
    fun `remapPath strips foreign prefix and reanchors under local root`() {
    ...
    fun `parse marks not_covered rows NONE so they count toward the total`() {
        ...
        // Non-statement rows stay null and are excluded from the denominator.
    ...
    @Test
    fun `parse reports a file with no covered rows at all`() {
        val json = """{"files":{"/x/rego/p.rego":{"covered":[],"not_covered":[
            {"start":{"row":2},"end":{"row":2}},{"start":{"row":3},"end":{"row":3}}
        ]}}}"""
        val cd = parser.parse(json, "rego", localRoot)
            .getClassData(Paths.get("/proj/rego/p.rego").toString())

        assertThat(cd).isNotNull
        assertThat(cd.getLineData(2).status.toInt()).isEqualTo(LineCoverage.NONE.toInt())
        assertThat(cd.getLineData(3).status.toInt()).isEqualTo(LineCoverage.NONE.toInt())
    }

    @Test
    fun `parse treats a row listed in both sets as covered`() {
        ...
        assertThat(cd.getLineData(4).status.toInt()).isEqualTo(LineCoverage.FULL.toInt())
        assertThat(cd.getLineData(6).status.toInt()).isEqualTo(LineCoverage.NONE.toInt())
    }
    ...
    fun `parse handles multiple files in the same report`() {
        ...
        // The unplanned rule's location spans its whole body, so every row of it is uncovered.
        for (row in 19..21) {
