...
/*
 * The engine/suite/runner triad the Coverage API requires. Each must be its own class because the
 * platform instantiates them by name, but they are only ever read together.
 */
...
class OpaIrCoverageEngine : CoverageEngine() {
    ...
    // Empty makes the framework fall through to getQualifiedNames(PsiFile) for gutter lookup rather
    // than getQualifiedName(File, PsiFile), whose default null silently disables coverage display.
...
class OpaIrCoverageSuite : BaseCoverageSuite {
    ...
    // getCoverageEngine() is declared non-null and called from places that cannot recover, so a
    // missing extension means a broken build; fail loudly rather than with first()'s
